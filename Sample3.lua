...
(1) たたみ込み	ReLU	filter:4	karnel:5*5 	zeroPad:なし	output:4*(W-6)*(W-6)
(3) Maxプーリング						karnel:3*3	zeroPad:なし	output:4*(W-8)*(W-8)
...
mlp:add(nn.Reshape(1 * 4 * (width-8) * (width-8)))
mlp:add(nn.Linear(1 * 4 * (width-8) * (width-8), 2*classes))
mlp:add(nn.ReLU())
