...
(2) Maxプーリング						karnel:3*3	zeroPad:なし	output:14*(W-6)*(W-6)
(3) 全結合層		ReLU										output:classes*3
(4) 全結合層		(Log)SoftMax								output:classes
誤差関数:交差エントロピー
]]
...
mlp:add(nn.Reshape(14 * (width-6) * (width-6)))
mlp:add(nn.Linear(14 * (width-6) * (width-6), 3*classes))
mlp:add(nn.ReLU())
