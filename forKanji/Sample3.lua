...
(1) たたみ込み	ReLU	filter:14	karnel:7*7 	zeroPad:なし	output:14*(W-6)*(W-6)
(3) Maxプーリング						karnel:3*3	zeroPad:なし	output:14*(W-8)*(W-8)
...
mlp = nn.Sequential()
mlp:add(nn.SpatialConvolutionMM(1, 1*14, 7, 7,1,1,0,0))	--(channel_in, channel_out, kW, kH)
mlp:add(nn.ReLU())
--mlp:add(nn.Dropout(0.2))
mlp:add(nn.SpatialMaxPooling(3,3,1,1,0,0))	--(kW, kH)
mlp:add(nn.Reshape(14 * (width-8) * (width-8)))
mlp:add(nn.Linear(14 * (width-8) * (width-8), 3*classes))
mlp:add(nn.ReLU())
